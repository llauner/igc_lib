...
def main(request):
    ...
    if 'relTargetDate' in request.args:
        ...
        now = date(script_start_time.year,
    ...
    print("##### Launching processing for: {} target_date={} relDaysLookup={} catchupOnPreviousDay={}".format(
        processName, target_date, relDaysLookup, catchupOnPreviousDay))
    ...
    date_suffix = str(target_date.year) + "_" + \
        str(target_date.month).zfill(2) + "_" + str(target_date.day).zfill(2)
    ...
    ftp_client_igc = FtpHelper.get_ftp_client(
        ftp_server_name_igc, ftp_login_igc, ftp_password_igc)

    # ---------------------------------------------------- Heatmap ----------------------------------------------
    if processingType == ProcessingType.HeatMap:

        # Get files to process
        all_files = FtpHelper.get_file_names_from_ftp(
            ftp_client_igc, target_date, relDaysLookup)
        ...
        if all_files:
            for i, filename in enumerate(all_files):
                ...
                if flight.valid and flight_date == target_date:
                    ...
                    print("{}/{} :{} -> {} \t Thermals#={}".format(i+1, files_count,
                                                                   filename, zip_file.filelist[0].filename, len(flight.thermals)))
                else:
                    print("{}/{} :{} -> {} \t Discarded ! valid={} date={}".format(i+1, files_count,
                                                                                   filename, zip_file.filelist[0].filename, flight.valid, flight_date))
                del flight
            ...
            if isOutputToGoogleCloudStorage:
                ...
                igc2geojson.dump_to_google_storage(
                    output_latest, global_thermals)
            ...
            ftp_client_out = FtpHelper.get_ftp_client(
                ftp_server_name, ftp_login, ftp_password)
            if not dry_run:
                igc2geojson.dump_to_ftp(
                    ftp_client_out, FTP_HEATMAP_ROOT_DIRECTORY, output_filename, global_thermals)
            ftp_client_out.close()
            print(
                "GeoJson output to FTP: {} -> {}.geojson".format(ftp_client_out.host, output_filename))
            ...
            if is_latest_processing:
                ...
                if not dry_run:
                    igc2geojson.dump_to_ftp(
                        ftp_client_out, FTP_HEATMAP_ROOT_DIRECTORY, output_filename_latest, global_thermals)
                ftp_client_out.close()
                print(
                    "GeoJson output to FTP: {} -> {}".format(ftp_client_out.host, output_filename_latest))

            # Metadata
            script_end_time = datetime.now(tz)
            metadata = RunMetadata()
            metadata.init(target_date, script_start_time,
                          script_end_time, flights_count, len(global_thermals))
            ...
            if is_latest_processing:
                ftp_client_out = FtpHelper.get_ftp_client(
                    ftp_server_name, ftp_login, ftp_password)
                if not dry_run:
                    igc2geojson.dump_string_to_ftp(
                        ftp_client_out, FTP_HEATMAP_ROOT_DIRECTORY, output_filename_metadata_latest, jsonMetadata)
                ftp_client_out.close()
                print("Metadata JSON output to FTP: {} -> {}".format(
                    ftp_client_out.host, output_filename_metadata_latest))
            ...
            if not dry_run:
                igc2geojson.dump_string_to_ftp(
                    ftp_client_out, FTP_HEATMAP_ROOT_DIRECTORY, output_filename_metadata, jsonMetadata)
            ftp_client_out.close()
            print("Metadata JSON output to FTP: {} -> {}".format(
                ftp_client_out.host, output_filename_metadata))
        else:
    ...
    # ---------------------------------------------------- Cumulative Track ----------------------------------------------
        ...
        cumulativeTrackBuilder = CumulativeTrackBuilder(
            ftp_client_igc, ftp_client_out, target_year, useLocalDirectory=True, isOutToLocalFiles=True)
